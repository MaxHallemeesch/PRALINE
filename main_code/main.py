...
if __name__ == '__main__':
    ...
    parser.add_argument('--gene_embeddings', type=str, default='gene_embeddings/gene_embeddings_normalized.npy', help='path to the file that contains the gene embeddings')
    parser.add_argument('--lam', type=int, default=0, help='the value of lambda hyperparameter')
    parser.add_argument('--testing', type=int, default=1, help='testing')
    ...
    train_idxs, val_idxs, test_idxs = patient_split(df)
    ...
    for train_idx, val_idx, test_idx in zip(train_idxs, val_idxs, test_idxs):
        ...
        if args.checkpoint and args.change_num_genes: # if finetuning from model trained on gtex/tcga
            ...
            model = ViT_PriorKnowledge(num_outputs=args.num_genes, dim=2048, depth=6, heads=16, mlp_dim=2048, dim_head = 64, lam=args.lam, gene_embeddings=gene_embeddings) 
            model.load_state_dict(torch.load(model_path))
            
            model.linear_head = nn.Sequential(
                nn.LayerNorm(2048),
                PriorKnowledgeLinear(2048, num_outputs, lam=args.lam, X=gene_embeddings)
        )
        ...
        preds, real, wsis, projs = evaluate(model, val_dataloader, run=run, suff=f'_val_{i}')
        print()

        print("Printing Test Results")
        preds, real, wsis, projs = evaluate(model, test_dataloader, run=run, suff=f'_{i}')
