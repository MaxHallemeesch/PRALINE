...
class PriorKnowledgeLinearBalanced(nn.Module):
    def __init__(self, hidden_dim, num_outputs, lam, gene_embeddings, device='cuda'):
        super().__init__()
        self.Y_init = torch.empty((num_outputs, hidden_dim)).uniform_(-math.sqrt(1/hidden_dim), to=math.sqrt(1/hidden_dim))
        self.Y = Parameter(self.Y_init.to(device))
        self.bias_init = torch.empty(num_outputs).uniform_(-math.sqrt(1/hidden_dim), to=math.sqrt(1/hidden_dim))
        self.bias = Parameter(self.bias_init.to(device))
...
class ViTPriorKnowledgeBalanced(nn.Module):
...
class MLPPriorKnowledgeBalanced(nn.Module):
    def __init__(self, *, num_outputs, dim, lam, gene_embeddings,
                num_clusters=100, device='cuda'):
        super().__init__()
...
def evaluate(model, dataloader, dataset, run=None, verbose=True, suff=''):
    model.eval()
    loss_fn = nn.MSELoss()
    losses = []
    preds = []
